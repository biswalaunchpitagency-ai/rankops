...
export default function ReplyForm({ ticket }: ReplyFormProps) {
  ...
  const suggestMutation = useMutation({
    mutationFn: async () => {
      const { data } = await axios.post(`/api/tickets/${ticketId}/replies/suggest`);
      return data.body as string;
    },
    onSuccess: (suggestedText) => {
      setValue("body", suggestedText, { shouldValidate: true });
    },
  });
  ...
  return (
    <form onSubmit={handleSubmit((data) => replyMutation.mutate(data))} className="space-y-3">
      ...
      {suggestMutation.error && (
        <ErrorAlert error={suggestMutation.error} fallback="Failed to generate suggestion" />
      )}
      ...
      <div className="flex gap-2">
        <Button
          type="button"
          variant="outline"
          disabled={suggestMutation.isPending || replyMutation.isPending || polishMutation.isPending}
        ...
        <Button
          type="button"
          variant="outline"
          disabled={!bodyValue?.trim() || polishMutation.isPending || replyMutation.isPending || suggestMutation.isPending}
          onClick={() => polishMutation.mutate()}
        >
          {polishMutation.isPending ? "Polishing..." : "Polish"}
        </Button>
        <Button type="submit" disabled={!bodyValue?.trim() || replyMutation.isPending || polishMutation.isPending || suggestMutation.isPending}>
